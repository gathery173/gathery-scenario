...
function render() {
  ...
  data["content"].forEach(i => {
    console.log(i)
      if (i["type"] === "text") {
        pl.innerHTML += `<div class="list__item is-idle js-item text">
        <div style="width: 100%;">
          <div class="form-floating">
            <input type="text" class="form-control actor-input" placeholder="Ім'я" value="${i["content"]["name"]}">
            <textarea class="textarea resize-ta mt-2">${i["content"]["text"]}</textarea>
          </div>
        </div>
      ...
      <div class="delete-item-btn">
        ...
        pl.innerHTML += `<div class="list__item is-idle js-item music">
        <div class="form-floating d-flex" style="width: 100%;">
          ...
          <input type="text" class="form-control actor-input" placeholder="Музика" value = "${i["content"]}">
        </div>
        <div class="d-flex actions">
          <div class="drag-handle js-drag-handle"></div>
          <div>
        ...
        <div class="form-floating d-flex" style="width: 100%;">
          ...
          <input type="text" class="form-control actor-input" placeholder="Коментар" value ="${i["content"]}">
...
function logCurrentOrder() {
  const currentOrder = getAllItems();

  console.log("-".repeat(10))
  var result = []
  currentOrder.forEach(item => {

      /*const itemNameElement = item.querySelector('.actor-input');
      const itemTextElement = item.querySelector('.resize-ta');

      // Check if the elements are found
      if (itemNameElement && itemTextElement) {
          const itemName = itemNameElement.value.trim();
          const itemText = itemTextElement.value.trim();
          
          console.log(`Name: ${itemName}, Text: ${itemText}`);
      } else {
          console.error("Error: Element not found for logging order");
      }*/
      
      if (Array.from(item.classList).includes("text")) {
        const itemNameElement = item.querySelector('.actor-input');
        const itemTextElement = item.querySelector('.resize-ta');
        console.log("Text ----")
        console.log(itemNameElement.value.trim())
        console.log(itemTextElement.value.trim())
        result.push({
          type: "text",
          content: {
            name: itemNameElement.value.trim(),
            text: itemTextElement.value.trim()
          }
        })
      } else if (Array.from(item.classList).includes("comment")) {
     
        console.log("Comment ----")
        console.log(item.querySelector(".actor-input").value.trim())
        result.push({
          type: "comment",
          content: item.querySelector(".actor-input").value.trim()
        })
       
      } else if (Array.from(item.classList).includes("music")) {
  ...
  console.log(result)
      set(ref(db, `scenarios/${url.get("id")}/content`), result)
}
document.getElementById("logOrderButton").addEventListener("click", logCurrentOrder)
